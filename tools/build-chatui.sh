...
if [ "${1:-}" = "--kindle" ]; then
    TC="${KOX_TC:-$HOME/x-tools/arm-kindlehf}"
    if [ -z "${CC:-}" ]; then
    ...
    if ! pkg-config --exists gtk+-3.0; then
        echo "error: gtk+-3.0 is not in the koxtoolchain sysroot" >&2
        exit 1
    fi
    # shellcheck disable=SC2046
    "$CC" --sysroot "$SYSROOT" -O2 -Wall -Wextra -s \
        $(pkg-config --cflags gtk+-3.0) -o "$OUT/chat-ui-kindle" "$SRC" \
        $(pkg-config --libs gtk+-3.0)
    echo "  built out/chat-ui-kindle ($(du -h "$OUT/chat-ui-kindle" | cut -f1))"
    exit 0
fi
